...
public abstract class Piece : MonoBehaviour
{
    ...
    {   //setup variables
        LinkComponentsAndSetupVariables();
        DefineFront(); //define front directions
    }
    private void LinkComponentsAndSetupVariables()
    {
    ...
    private void Start()
    {
        ...
        //set marker grid based on team color but not dependent on specific colors //REVISIT THIS TO MAKE IT MORE FLEXIBLE
        SetMarkerGridBasedOnTeamColor();

        //MAKE SOLDIERS: soldier generation in formation
    ...
    private void SetMarkerGridBasedOnTeamColor() //give this piece a markergrid based on team color. right now only works with two marker grids, implying binary teams. ie no support for free for alls
    {
        int num = 0;
    ...
    private void StartSoldierGeneration()
    {
        if (models == 1)
        {
    ...
    public void ClearQueuedMoves()
    {
        ...
        board.ClearMoves(unitID); //onClearMoves inside of Board
    }
    ...
    public void OnUpdateTerrainType(int x, int y) //Update unit's terrain and its stats accordingly
    {
    ...
    public void OnRunThroughCylinders()
    {
        ...
        //Debug.Log(tileDistance + " tile distance"); 
        //CHECK ARRAYS TO SEE HOW FAR AWAY: we can check to see if it's within our existing arrays to find how far it is away;
        {
            bool foundIt = false;
            foreach (var item in adjacentTiles) //first look through here //REVISIT? THIS IS VERY DUMB
            {
